...
dbpromise.then(function(db){
  ...
  return fetch(request).then(networkResponse =>{
    dbpromise.then(function(db){
      ...
      var keyValStore = tx.objectStore('restaurant');
      keyValStore.oncomplete = function() { console.log('completed'); }; 
      keyValStore.onabort = function() { console.log('aborted'); }
      console.log(networkResponse.clone());
      keyValStore.put("networkResponse.clone()",'test').catch(error => {console.log("Request Save Error:"+error);});
