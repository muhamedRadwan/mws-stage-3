...
dbPromise.then(function(db){
  ...
  return keyValStore.get('keyval');
}).then(function(blobFound){
  if (!blobFound) {
    console.log(`error in retrieving from db: ${blobFound}`);
    ...
    return fetch(request.clone())
      .then((response) => {
        ...
        response.clone().blob().then(
          (blob) => {
            console.info(`updating cache with: ${JSON.stringify(request.clone().url)}, then returning`);
            idbKeyval.set(
              request.url,
              blob
            ).then(
  ...
  const contentType = consts.getBlobType(blobFound, request.url);
  console.log('responding from cache', request.url, contentType);
  // on this page https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API/Using_Fetch
  const myHeaders = new Headers({
    "Content-Length": String(blobFound.size),
    "Content-Type": contentType,
